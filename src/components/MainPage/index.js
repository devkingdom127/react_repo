...
import {getExtraOptions} from "./linkOptions/utils"
...
export const MainPage = ({ isLoggedIn, login, logout }) => {
  ...
  const longUrlPasteHandler = (e) => {
    ...
    setLongUrl(value);
    let https = value !== undefined && value.slice(0, 5);

    if (https === "https" && value.length > 10) {
      setError("");
      setValidUrl(true);
    } else {
      setError("Please paste a valid https link.");
      setValidUrl(false);
    }
